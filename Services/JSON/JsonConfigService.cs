...
public class JsonConfigService : IJsonConfigService
{
    ...
    private IEnumerable<string> GetConfigs()
    {
        ...
        var not_blacklisted = new Spec<string>(filepath =>
        ...
        var configs = grepper.GetFileNames().Where(not_blacklisted)
        // .Dump("configs")
        ;
        return configs;
    }
    ...
    private void CheckForWatchedConfigFiles()
    {
        ...
        string cs_projname = "";
        Console.WriteLine(
            $"WARNING: Could not find 'config.json' files in a <Watch> inside project '{cs_projname}'"
        );
    }
