...
public class HTMXTable<T>
{
    ...
    public Dictionary<int, IList<Func<T, T>>> Sorts { get; set; } =
        new Dictionary<int, IList<Func<T, T>>>(); //TODO: use the If<T>(...) method you found

    public Dictionary<int, IList<Func<T, T>>> Filters { get; set; } =
        new Dictionary<int, IList<Func<T, T>>>(); //TODO: implement that awesome Enum code you found which makes strings to Enum support easy, OR install a Nuget you found that does the same.
